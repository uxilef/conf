...
echo "[3/5] Lese permanentes Master-Token aus der Datenbank-Config..."
TOKEN=$(grep 'manager_token:' /opt/kasm/current/conf/database/seed_data/default_properties.yaml | awk '{print $2}' | tr -d '"' | tr -d "'")
...
sed -i -E 's|^([[:space:]]*token:).*|\1 "'"$TOKEN"'"|' /opt/kasm/current/conf/app/kasmguac.app.config.yaml
sed -i -E 's|^([[:space:]]*token:).*|\1 "'"$TOKEN"'"|' /opt/kasm/current/conf/app/rdp_gateway.app.config.yaml
sed -i -E 's|^([[:space:]]*token:).*|\1 "'"$TOKEN"'"|' /opt/kasm/current/conf/app/rdp_https_gateway.app.config.yaml
